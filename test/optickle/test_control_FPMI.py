"""
Unit tests for Optickle MIMO control of FPMI
"""
...
opt = optFPMI.optFPMI(eng, 'opt', par)

# Frequency vector for calculations
fmin = 0.3
fmax = 1e3
npts = 400
...
probesDARM = {'AS_Q': 1/np.abs(opt.getTF('AS_Q', DARM)[0])}
probesCARM = {'REFL_I': 1/np.abs(opt.getTF('REFL_I', CARM)[0])}
probesBS   = {'REFL_Q': 1/np.abs(opt.getTF('REFL_Q', 'BS')[0])}
...
shot_noise = {probe: opt.getQuantumNoise(probe) for probe in cs.probes}
